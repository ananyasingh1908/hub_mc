...
async function findEmployeeByEmail(email: string): Promise<EmployeeResult | null> {
  ...
  const role = row.userRole as "EMPLOYEE" | "SUPER_ADMIN";
...
async function verifyGoogleToken(
  ...
  try {
    ...
    if (payload.aud !== GOOGLE_CLIENT_ID) {
      console.error(
        "[EmployeeAuth] Token audience mismatch: got",
        payload.aud,
        "expected",
        GOOGLE_CLIENT_ID,
      );
...
export async function handleEmployeeLoginRequest(
  request: Request,
): Promise<Response> {
  ...
  if (!employee) {
    console.warn(
      "[EmployeeAuth] Unauthorized employee account for email:",
      googleUser.email,
    );
    return Response.json(
